...
Four infrastructure components deployed in order:

1. **PostgreSQL** — shared database cluster via CloudNativePG operator. Each application gets its own database and credentials. Future services (Gitea, Infisical) connect as clients.
2. **Authentik server** — identity provider, admin UI, user self-service portal at `auth.catinthehack.ca`. Connects to shared PostgreSQL (Redis removed in Authentik v2025.10).
3. **Proxy outpost** — separate Deployment running `ghcr.io/goauthentik/proxy`. Handles forward-auth requests from Traefik.
4. **Kyverno mutating policy** — auto-injects the Traefik ForwardAuth middleware annotation into every HTTPRoute unless explicitly opted out.
...
Future services (Gitea, Infisical) require PostgreSQL. Running one shared cluster with per-app databases avoids duplicating infrastructure. Lightweight on a single-node cluster.

### Why CloudNativePG over Bitnami PostgreSQL

Bitnami's free Helm chart registry (Docker Hub OCI) froze in August 2025 — no new chart or container image updates. CloudNativePG is actively maintained, Kubernetes-native, and supports declarative database/role management via CRDs for future services.
...
PostgreSQL lives in its own namespace as shared infrastructure. The CloudNativePG operator manages the cluster from `cnpg-system`. Each consumer gets isolated credentials and a dedicated database.
...
│   ├── secret.enc.yaml             # secret key, bootstrap token, DB creds
...
│   ├── middleware-forward-auth.yaml              # Traefik ForwardAuth Middleware
...
```
cloudnative-pg operator ─► postgres cluster ─► authentik
                                                   │
                                                   ▼
                                              proxy outpost
                                              (manual Deployment)
...
The CNPG operator must install CRDs before the PostgreSQL cluster can be created, and PostgreSQL must be ready before Authentik starts.

## Blueprints

Four blueprints stored in a single ConfigMap, mounted at `/blueprints/mounted/cm-authentik-blueprints/`.
...
`forward_domain` mode protects all domains under `*.catinthehack.ca` through one outpost with a shared cookie. No per-app provider definitions needed.
...
1. **CloudNativePG operator** — namespace, HelmRepository, HelmRelease in `cnpg-system`. Verify: operator pod running, CRDs installed.
2. **PostgreSQL cluster** — namespace, CNPG cluster HelmRelease (cross-namespace sourceRef), SOPS-encrypted credentials. Verify: pod running in `postgres` namespace, `authentik` database exists.
3. **Authentik** — namespace, HelmRepository, HelmRelease, SOPS secret (secret key, bootstrap token, DB credentials), HTTPRoute, blueprints ConfigMap. Verify: `auth.catinthehack.ca` loads the login page, enrollment flow works, admin account accesses the admin UI.
4. **Proxy outpost** — Deployment, Service, outpost Secret, outpost HTTPRoute for callback paths. Verify: outpost logs show successful connection to Authentik, forward-auth endpoint responds.
5. **Default-deny middleware** — Traefik Middleware CRD (in cluster-policies), Kyverno mutating ClusterPolicy, opt-out label on Authentik's HTTPRoute. Verify: unauthenticated request to `whoami.catinthehack.ca` redirects to `auth.catinthehack.ca`, login succeeds and redirects back.

## Prerequisites

1. **Authentik database in PostgreSQL** — created via CloudNativePG cluster chart `initdb` configuration. Credentials stored in SOPS.
...
4. **`/etc/hosts` entry** — `auth.catinthehack.ca` pointing to `192.168.20.100`.
...
- **Steps 13–15** — Gitea, Renovate, and Infisical use the shared PostgreSQL and Authentik OIDC
- **New convention** — shared PostgreSQL is cluster infrastructure; future apps connect as clients with per-app databases and credentials
